The user wants to optimize the README.md file. Let me first read it to see the current content.

先确认一下项目实际有哪些文件，保证 README 和实际项目一致
The user wants to optimize the README.md. Let me analyze the current README and identify issues:

1. The directory structure says `mall-deploy/` but the actual project is `mall-primary-back-end`
2. References `docker-compose.yml` but the actual file is `compose.yaml` (Docker Compose V2 naming)
3. Uses `docker-compose` command (V1) instead of `docker compose` (V2)
4. The `sql/init.sql` is referenced but may not exist
5. Some outdated or inconsistent naming
6. The new Dockerfile uses multi-stage build, so the README could mention that

Let me check what files actually exist in the project to make sure the README matches reality.

Let me try a different approach to list files.

Now I have a clear picture of the actual project structure. Let me identify the issues in the current README:

1. **目录结构不匹配**: README says `mall-deploy/` but actual project is `mall-primary-back-end`
2. **文件名不匹配**: README references `docker-compose.yml` but actual file is `compose.yaml`
3. **命令格式**: README uses `docker-compose` (V1) but should use `docker compose` (V2) since the file is `compose.yaml`
4. **Dockerfile 未提及**: The project has a `Dockerfile` with multi-stage build, but README doesn't mention it
5. **`.env.example` 未提及**: README mentions `.env` but not `.env.example`
6. **`DESIGN.md` 未提及**: There's a `DESIGN.md` file in the project
7. **`.gitignore` / `.gitattributes`**: Not important for README
8. **`Dockerignore`**: Could be mentioned
9. **`mall-primary-back-end.zip`**: Not relevant for README
10. **lib/ 目录**: Those jars are local dependencies, could be noteworthy
11. **`sql/init.sql`**: Exists, good

Let me now propose an optimized README. The key improvements:
- Fix the directory structure to match reality
- Change `docker-compose.yml` to `compose.yaml`
- Change `docker-compose` commands to `docker compose`
- Add Dockerfile mention
- Add `.env.example` mention
- Add link to DESIGN.md
- Make it more concise and accurate
- Add the new Dockerfile multi-stage build info

Let me create the diff.
看了项目实际文件结构，发现 README 有几处和实际项目不一致的地方。以下是优化方案：
...
| 问题 | README 当前 | 实际项目 |
|------|------------|---------|
| 项目目录名 | `mall-deploy/` | `mall-primary-back-end/` |
| 编排文件名 | `docker-compose.yml` | `compose.yaml` |
| 命令格式 | `docker-compose` (V1) | 应统一为 `docker compose` (V2) |
| Dockerfile | 未提及 | 存在，多阶段构建 |
| `.env.example` | 未提及 | 存在 |

由于改动较多，以下是优化后的完整 README：
